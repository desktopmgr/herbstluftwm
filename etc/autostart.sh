...
# @File          : autostart.sh
# @Description   : autostart script for herbsluftwm
# @TODO          :
# @Other         :
# @Resource      :
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
#set opts
# set desktop session
DESKTOP_SESSION="${DESKTOP_SESSION:-herbsluftwm}"
# set config dir
...
# Help
if [[ "$1" = *help ]]; then
  printf "\n\t\t%s\n" "Usage: $PROG" "Starts applications for herbsluftwm window manager"
  exit
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ -z "$DBUS_SESSION_BUS_ADDRESS" ]; then
  if __does_cmd_exist dbus-launch; then
    ...
    herbsluftwm) dbus_args+="herbsluftwm" ;;
